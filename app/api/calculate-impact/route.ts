...
export async function POST(request: NextRequest) {
  try {
    ...
    const { goal_title, goal_description, duration_days, selectedCountry } = body

    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" })

    const prompt = `
You are an environmental impact calculator specializing in rural and regional areas. Calculate the realistic environmental impact of this goal:
